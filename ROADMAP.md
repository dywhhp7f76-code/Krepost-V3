...
### Tool-output guard: проверка результатов инструментов/MCP  🔜
- **Что:** санитизация ответов инструментов ДО подачи в модель; фильтрация instruction-подобных спанов (в исследовании compliance падает 88%→13%); детект скрытых команд в возвращённых данных.
- **Откуда:** defense/2026-07-01 (Self-Study span filtering), defense/2026-07-02 (скрытая инструкция в ответе mcp-server-fetch).
- **К чему относится:** defense — расширение Layer 1/2 на **промежуточные** tool-call результаты (сейчас покрыты только вход и финальный выход).
- **Почему не сейчас:** MCP-инструментов между агентами ещё нет; берём на опережение, как только появятся tool calls. (Ранее помечено как gap в ревью.)

### Защита fetch-слоя: SSRF / обход URL-валидации  🔜
- **Что:** проверка URL перед fetch (запрет внутренних адресов, разбор credentials-части URL, нормализация схемы) — Крепость-клиент не должна ходить по вредоносным ссылкам из чужого контента.
- **Откуда:** redteam/2026-07-02 (URL Validation Bypass Cheat Sheet, Concealing payloads in URL credentials).
- **К чему относится:** defense — но для **клиентской** роли (fetch источников), а не сервера. Касается и news-бота, и будущего «облачного разведчика».
- **Почему не сейчас/near-term:** актуально уже с текущим fetch в боте; в самой Крепости — как только она начнёт фетчить облака.

### Нормализация до фильтрации: тест против Unicode-overflow и control-символов  🔜
- **Что:** пробник, проверяющий, что `normalize.py` не пробивается overflow-кодпоинтами (>255 усекаются в один байт) и управляющими ASCII-символами ДО фильтрации.
- **Откуда:** defense/2026-07-02 (Bypassing character blocklists with unicode overflows; Drag and Pwnd).
- **К чему относится:** defense — валидация уже существующего Layer 1 (`normalize.py`, NFKC-до-фильтрации).
- **Почему не сейчас/near-term:** дёшево и бьёт в наш threat model напрямую — кандидат на ближайший пробник.
